...
<html>
<head>
<c:set var="headKeywords" value="${warehouse.tags}"/>
<c:set var="headTitle" value="${warehouse.name}"/>
...
<body>
	<div class="main">
		...
		<section class=" pd10 bdb">
			<div>
				<span class="f18 pd10">${warehouse.name }</span><span class="flor f14">热线电话：<span
						class="f999">${warehouse.phoneNum }</span></span>
				<div class="pdY5">
				<span class="f14 f999 "><fmt:formatDate pattern="yyyy/MM/dd" value="${warehouse.updateDate }" />${dateDesc }</span>
				</div>
			</div>
		</section>
		<section class=" pd10 f14 mt10 bdt bdb lh105">
			<div class="detailMsg">
				<p>
					面积：<span class="f999">约${warehouse.grossFloorArea}㎡</span>
				</p>
				<p>
					区域：<span class="f999">${warehouse.region }</span>
				</p>
				<p>
					类别：<span class="f999">${warehouse.type }</span>
				</p>
				<c:if test="${warehouse.rentOrSale == '出租' or warehouse.rentOrSale == '不限'}">
					<p>
						在租：<span class="fc00">${warehouse.rental }元/月</span>
					</p>
				</c:if>
				<c:if test="${warehouse.rentOrSale == '出售' or warehouse.rentOrSale == '不限'}">
					<p>
						在售：<span class="fc00">${warehouse.totalPrice }万/元</span>
					</p>
				</c:if>
				<div class="l"></div>
				<h3>地址</h3>
				<p class="f999">${warehouse.address }</p>
				<div class="l"></div>
				<h3>描述</h3>
				<p class="f999">${warehouse.introduction }</p>

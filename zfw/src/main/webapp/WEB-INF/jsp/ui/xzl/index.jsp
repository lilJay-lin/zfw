...
<html>
<head>
<c:set var="headKeywords" value="写字楼"/>
<c:set var="headTitle" value="写字楼"/>
	...
	<div class="main">
			...
			<div class="searbox_new">
				<div class="ipt">
					<input type="search" name="keyword" id="keyWord" value="${keyWord }"
						placeholder="写字楼名/地名" autocomplete="off">
				...
				<ul id="criteriaList" class="criteriaList">
					...
					<li>
						<p>
							...
							<span class="js-sift-condition none" data-condition=""> 不限</span>
							<span class="js-sift-condition" data-condition="1:99"> 100以下</span>
							<span class="js-sift-condition" data-condition="100:199"> 100-199 </span>
							<span class="js-sift-condition" data-condition="200:299"> 200-299 </span>
							<span class="js-sift-condition" data-condition="300:399"> 300-399 </span>
							<span class="js-sift-condition" data-condition="400:499"> 400-499</span>
							<span class="js-sift-condition" data-condition="500:599"> 500-599</span>
							<span class="js-sift-condition" data-condition="600:699"> 600-699</span>
							<span class="js-sift-condition" data-condition="700:10000"> 700以上</span>
					...
					<li>
						<p>
							...
							<span class="js-sift-condition none" data-condition=""> 不限</span>
							<span class="js-sift-condition" data-condition="纯写字楼"> 纯写字楼 </span>
							<span class="js-sift-condition" data-condition="商住楼"> 商住楼 </span>
							<span class="js-sift-condition" data-condition="商业综合体楼"> 商业综合体楼</span>
							<span class="js-sift-condition" data-condition="酒店写字楼"> 酒店写字楼</span>
		...
		<section class="homeList whitebg bdb">
			<ul id="itemList">
				...
				<li><a id="${t.id}" href="${ctx}/xzl/${t.id }/detail">
						<div class="img">
		...
		<c:choose>
			<c:when test="${total!=0}">
				<div id="drag" class="cenBtn">
					<a class="draginner" href="javascript:void(0);">查看更多写字楼</a>
				</div>
			</c:when>
			<c:otherwise>
				<div class="searchNo">
			    	<p class="f14">暂未搜索到符合条件的写字楼。</p>
				</div>
			</c:otherwise>
		</c:choose>
...
var curp = 1;
	...
	function load(){
		...
		draginner.html("正在加载...");
		var url = "${ctx}/xzl/json/{keyWord}-{region}-{grossFloorArea}-{type}-{rentOrSale}-{rental}-{totalPrice}-{orderBy}-{targetPage}-{pageSize}/search";
			...
			success: function(json){
				if(json.success){
					for(var i=0;i<json.results.length;i++){
						...
						var str = '<li><a id="{id}" href="${ctx}/xzl/{id}/detail"> <div class="img"> <img src="{preImageUrl}"> </div> <div class="txt"> <p class="x-intro"> <h2>{name}</h2> <span class="new">{price}</span> </p> <p>{grossFloorArea}</p> <div class="stag"> <span class="t1">{address} </div> </div> </a></li>';
						var price = "";
	...
	function searchAction(){
		var url = "${ctx}/xzl/{keyWord}-{region}-{grossFloorArea}-{type}-{rentOrSale}-{rental}-{totalPrice}-{orderBy}/search";
		url = bindSearchData(url);

...
<html>
<head>
<c:set var="headKeywords" value="${ob.tags}"/>
<c:set var="headTitle" value="${ob.name}"/>
...
<body>
	<div class="main">
		...
		<section class=" pd10 bdb">
			<div>
				<span class="f18 pd10">${ob.name }</span><span class="flor f14">热线电话：<span
						class="f999">${ob.phoneNum }</span></span>
				<div class="pdY5">
				<span class="f14 f999 "><fmt:formatDate pattern="yyyy/MM/dd" value="${ob.updateDate }" />${dateDesc }</span>
				</div>
			</div>
		</section>
		<section class=" pd10 f14 mt10 bdt bdb lh105">
			<div class="detailMsg">
				<p>
					面积：<span class="f999">约${ob.grossFloorArea}㎡</span>
				</p>
				<p>
					装修：<span class="f999">${ob.decorationStatus }</span>
				</p>
				<p>
					区域：<span class="f999">${ob.region }</span>
				</p>
				<p>
					类别：<span class="f999">${ob.type }</span>
				</p>
				<c:if test="${ob.rentOrSale == '出租' or ob.rentOrSale == '不限'}">
					<p>
						在租：<span class="fc00">${ob.rental }元/月</span>
					</p>
				</c:if>
				<c:if test="${ob.rentOrSale == '出售' or ob.rentOrSale == '不限'}">
					<p>
						在售：<span class="fc00">${ob.totalPrice }万/元</span>
					</p>
				</c:if>
				<p>
					物业费：<span class="f999">${ob.propertyFee }元/平方米·月</span>
				</p>
				<div class="l"></div>
				<h3>地址</h3>
				<p class="f999">${ob.address }</p>
				<div class="l"></div>
				<h3>描述</h3>
				<p class="f999">${ob.introduction }</p>

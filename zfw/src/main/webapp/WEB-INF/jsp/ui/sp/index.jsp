	...
	<div class="main">
			...
			<div class="searbox_new">
				<div class="ipt">
					<input type="search" name="keyword" id="keyWord" value="${keyWord }"
						placeholder="商铺名/地名" autocomplete="off">
				...
				<ul id="criteriaList" class="criteriaList">
					...
					<li>
						<p>
							...
							<span class="js-sift-condition none" data-condition=""> 不限</span>
							<span class="js-sift-condition" data-condition="住宅底商"> 住宅底商</span>
							<span class="js-sift-condition" data-condition="商业街商铺"> 商业街商铺 </span>
							<span class="js-sift-condition" data-condition="临街门面"> 临街门面 </span>
							<span class="js-sift-condition" data-condition="写字楼配套底铺"> 写字楼配套底铺 </span>
							<span class="js-sift-condition" data-condition="购物中心/百货"> 购物中心/百货</span>
		...
		<section class="homeList whitebg bdb">
			<ul id="itemList">
				...
				<li><a id="${t.id}" href="${ctx}/sp/${t.id }/detail">
						<div class="img">
...
var curp = 1;
	...
	function load(){
		...
		draginner.html("正在加载...");
		var url = "${ctx}/sp/json/{keyWord}-{region}-{grossFloorArea}-{type}-{rentOrSale}-{rental}-{totalPrice}-{orderBy}-{targetPage}-{pageSize}/search";
			...
			success: function(json){
				if(json.success){
					for(var i=0;i<json.results.length;i++){
						...
						var str = '<li><a id="{id}" href="${ctx}/sp/{id}/detail"> <div class="img"> <img src="{preImageUrl}"> </div> <div class="txt"> <p class="x-intro"> <h2>{name}</h2> <span class="new">{price}</span> </p> <p>{grossFloorArea}</p> <div class="stag"> <span class="t1">{address} </div> </div> </a></li>';
						var price = "";
	...
	function searchAction(){
		var url = "${ctx}/sp/{keyWord}-{region}-{grossFloorArea}-{type}-{rentOrSale}-{rental}-{totalPrice}-{orderBy}/search";
		url = bindSearchData(url);

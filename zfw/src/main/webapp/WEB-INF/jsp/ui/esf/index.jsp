...
<html>
<head>
<c:set var="headKeywords" value="二手房"/>
<c:set var="headTitle" value="二手房"/>
	...
	<div class="main">
		...
		<section class="homeList whitebg bdb">
			<ul id="itemList">
				...
				<li><a id="${t.id}" href="${ctx}/esf/${t.id }/detail">
						<div class="img">
						...
						<div class="txt">
							<p class="x-intro">
							<h2>${t.name }</h2>
							<span class="new">${t.totalPrice }万/套</span>
							</p>
		...
		<c:choose>
			<c:when test="${total!=0}">
				<div id="drag" class="cenBtn">
					<a class="draginner" href="javascript:void(0);">查看更多二手房源</a>
				</div>
			</c:when>
			<c:otherwise>
				<div class="searchNo">
			    	<p class="f14">暂未搜索到符合条件的二手房源。</p>
				</div>
			</c:otherwise>
		</c:choose>
...
var curp = 1;
	...
	function load(){
		...
		draginner.html("正在加载...");
		var url = "${ctx}/esf/json/{residenceCommunityId}-{keyWord}-{region}-{totalPrice}-{roomNum}-{grossFloorArea}-{orderBy}-{targetPage}-{pageSize}/search";
			...
			success: function(json){
				if(json.success){
					for(var i=0;i<json.results.length;i++){
						...
						var str = '<li><a id="{id}" href="${ctx}/esf/{id}/detail"> <div class="img"> <img src="{preImageUrl}"> </div> <div class="txt"> <p class="x-intro"> <h2>{name}</h2> <span class="new">{totalPrice}万/套</span> </p> <p>{numMsgStr}</p> <div class="stag"> <span class="t1">{residenceCommunityName} </div> </div> </a></li>';
						var numMsgStr = "";
						...
						str = str.replace(/{totalPrice}/g,result.totalPrice);
	...
	function searchAction(){
		var url = "${ctx}/esf/{residenceCommunityId}-{keyWord}-{region}-{totalPrice}-{roomNum}-{grossFloorArea}-{orderBy}/search";
		url = bindSearchData(url);

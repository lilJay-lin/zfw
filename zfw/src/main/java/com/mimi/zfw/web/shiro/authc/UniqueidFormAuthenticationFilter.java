...
public class UniqueidFormAuthenticationFilter extends FormAuthenticationFilter {
	private static final Logger LOG = LoggerFactory.getLogger(UniqueidFormAuthenticationFilter.class);  
	    ...
	    protected void doCaptchaValidate(HttpServletRequest request,  
	        ...
	        if (token.getCaptcha() == null || !"mimi".equalsIgnoreCase(token.getCaptcha())) {  
	    ...
	    protected UniqueidUsernamePasswordToken createToken(ServletRequest request,  
	        ...
	        try {
//		        System.out.println(request.getParameter("publicExponent"));
//		        System.out.println(request.getParameter("modulus"));
//		        System.out.println(request.getParameter("password"));
	        	password = RSAUtil.getResult(request.getParameter("publicExponent"),request.getParameter("modulus"),request.getParameter("password"));
			} catch (Exception e) {
				e.printStackTrace();
			}
//	        System.out.println(password);
	    ...
	    protected void issueSuccessRedirect(ServletRequest request, ServletResponse response) throws Exception {
	        if(pathsMatch(getLoginMIUrl(),request)){
		        WebUtils.redirectToSavedRequest(request, response, getSuccessMIUrl());
	        }else{
	    ...
	    protected boolean isLoginRequest(ServletRequest request, ServletResponse response) {
	        return pathsMatch(getLoginUrl(), request) || pathsMatch(getLoginMIUrl(),request);
	    }
	    ...
	    protected void redirectToLogin(ServletRequest request, ServletResponse response) throws IOException {
	        ...
	        if(curUrl.indexOf("/"+miUrl+"/")!=-1 || (curUrl.lastIndexOf("/"+miUrl)+miUrl.length())==(curUrl.length()-1)){

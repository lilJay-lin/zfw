...
public class OfficeBuildingServiceImpl extends
    ...
    public void setBaseDao(
	    IBaseDao<OfficeBuilding, OfficeBuildingExample, String> baseDao) {
	...
	for (int i = 0; i < 50; i++) {
	    ...
	    ob.setPropertyFee(RandomUtils.nextInt(20) + RandomUtils.nextFloat());
	...
	resMap = checkOfficeBuilding(officeBuilding);
	if (!resMap.isEmpty()) {
	    return resMap;
	}
	officeBuilding.setId(UUID.randomUUID().toString());
	officeBuilding.setCreater(curUserId);
	officeBuilding.setLastEditor(curUserId);
	obm.insertSelective(officeBuilding);
	...
	officeBuilding.setLastEditor(curUserId);
	obm.updateByPrimaryKeySelective(officeBuilding);
	...
	String[] ids = OfficeBuildingIds.split(Constants.MI_IDS_SPLIT_STRING);
	List<String> list = new ArrayList<String>();
	for (String id : ids) {
	    list.add(id);
	}	
	String curUserId = userService.getCurUserId();
	OfficeBuildingExample example = new OfficeBuildingExample();

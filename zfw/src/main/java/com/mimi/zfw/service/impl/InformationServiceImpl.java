...
public class InformationServiceImpl extends
		BaseService<Information, InformationExample, String> implements
		IInformationService {
	...
	public void setBaseDao(
			IBaseDao<Information, InformationExample, String> baseDao) {
	...
	public Map<String, String> addInfo(Information info, String repIds) {
		...
		resMap = checkInfo(info);
		if(!resMap.isEmpty()){
			return resMap;
		}
		info.setId(UUID.randomUUID().toString());
		info.setCreater(curUserId);
		info.setLastEditor(curUserId);
		im.insertSelective(info);
		if (StringUtils.isNotBlank(repIds)) {
			String[] ids = repIds.split(Constants.MI_IDS_SPLIT_STRING);
			for (String repId : ids) {
				if (StringUtils.isNotBlank(repId)) {
					...
					rri.setInformationId(info.getId());
					rri.setRealEstateProjectId(repId);
	...
	public Map<String, String> updateInfo(Information info, String addREPs,
		...
		info.setLastEditor(curUserId);
		im.updateByPrimaryKeySelective(info);
		if (StringUtils.isNotBlank(addREPs)) {
			String[] ids = addREPs.split(Constants.MI_IDS_SPLIT_STRING);
			for (String repId : ids) {
				if (StringUtils.isNotBlank(repId)) {
					RelationREPAndInformation rri = new RelationREPAndInformation();
					rri.setId(UUID.randomUUID().toString());
		...
		if (StringUtils.isNotBlank(delREPs)) {
			String[] ids = delREPs.split(Constants.MI_IDS_SPLIT_STRING);
			List<String> delREPIds = new ArrayList<String>();
			for (String repId : ids) {
				if (StringUtils.isNotBlank(repId)) {
					delREPIds.add(repId);
				}
			}
			if(!delREPIds.isEmpty()){
				RelationREPAndInformationExample rrie = new RelationREPAndInformationExample();
				rrie.or().andInformationIdEqualTo(info.getId()).andRealEstateProjectIdIn(delREPIds).andDelFlagEqualTo(false);
				RelationREPAndInformation rri = new RelationREPAndInformation();
				rri.setDelFlag(true);
				rri.setLastEditor(curUserId);
	...
	public Map<String, String> batchDel(String infoIds) {
		...
		if(!idList.isEmpty()){
			...
			info.setLastEditor(curUserId);
			im.updateByExampleSelective(info, ie);
			
			RelationREPAndInformationExample rrie = new RelationREPAndInformationExample();
			rrie.or().andInformationIdIn(idList).andDelFlagEqualTo(false);
			RelationREPAndInformation rri = new RelationREPAndInformation();
			rri.setDelFlag(true);
			rri.setLastEditor(curUserId);
			rrim.updateByExampleSelective(rri, rrie);

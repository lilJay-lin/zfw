...
//		return kp;
	...
	public static String getCurrentModulus(){
		if(RSAUtil.pairs.isEmpty()){
			try {
	...
	public static String getCurrentPublicExponent(){
		if(RSAUtil.pairs.isEmpty()){
			try {
				RSAUtil.generateKeyPair();
			} catch (Exception e) {
	...
	public static String getFitPrivateExponent(String publicExponent,String modulus){
		for(int i=0;i<RSAUtil.pairs.size();i++){
			RSAPublicKey rpu = (RSAPublicKey)RSAUtil.pairs.get(i).getPublic();
			if(rpu.getPublicExponent().toString(16).equals(publicExponent) && rpu.getModulus().toString(16).equals(modulus)){
				RSAPrivateKey rpri = (RSAPrivateKey)RSAUtil.pairs.get(i).getPrivate();
...
//		byte[] de_test2 = RSAEncrypt.decrypt((RSAPrivateKey) getKeyPair().getPrivate(), en_test);

...
public class PGController {
	...
	public @ResponseBody Object analyse(HttpServletRequest request,String residenceCommunityName,String residenceCommunityId,Float grossFloorArea,String forward,Integer curFloor,Integer plusValue) {
		JSONObject jo = new JSONObject();
		try{
			Map<String,Object> resultMap = shhfplfService.analyse(residenceCommunityId,residenceCommunityName,forward,curFloor,plusValue);
			if(StringUtils.isBlank((String) resultMap.get("error"))){

				int zfTotalNum = rhService.countRentalHousingByParams(null, null, null,
						null, null, null);
				int esfTotalNum = shhService.countSecondHandHouseByParams(null, null, null,
						null, null, null);
				int averagePrice = 0;
	...
	public @ResponseBody Object miniAnalyse(HttpServletRequest request,String residenceCommunityName,String residenceCommunityId,Float grossFloorArea,String forward,Integer curFloor) {
		JSONObject jo = new JSONObject();
		try{
			Map<String,Object> resultMap = shhfplfService.analyse(residenceCommunityId,residenceCommunityName,forward,curFloor,0);
			if(StringUtils.isBlank((String) resultMap.get("error"))){
				int price = (int) resultMap.get("price");
				int totalPrice = (int) (price*grossFloorArea/10000);
				jo.put("totalPrice", totalPrice);
				jo.put("success", true);
			}else if(Constants.ASSESSMENT_ERROR_NO_RESULT.equals(resultMap.get("error"))){
				jo.put("success", false);
			}else{
				jo.put("success", false);
				jo.put("msg", resultMap.get("error"));
